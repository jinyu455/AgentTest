...
输出：句面情绪(surface_emotion) → 真实情绪(true_emotion)
示例：surface="开心" → true="厌烦"
```
...
输出：主情绪(primary) + 次情绪(secondary) + 比例(mix_ratio)
示例：primary="疲惫"(0.58) + secondary="开心"(0.42)
```
...
| 情况                | 条件                                             | 情绪来源                | 强度来源                     | 置信度公式                  |
| ------------------- | ------------------------------------------------ | ----------------------- | ---------------------------- | --------------------------- |
| 讽刺确认 + 高置信度 | `is_sarcasm=True` 且 `sarcasm_confidence ≥ 0.65` | Sarcasm 的 true_emotion | Sarcasm 的 revised_intensity | `emotion×0.3 + sarcasm×0.7` |
| 讽刺确认 + 低置信度 | `is_sarcasm=True` 且 `sarcasm_confidence < 0.65` | 回退到 Emotion Agent    | Emotion 的 intensity         | `emotion×0.8`（降低 20%）   |
| 未检测到讽刺        | `is_sarcasm=False`                               | Emotion Agent           | Emotion 的 intensity         | `emotion×0.9`（降低 10%）   |

**混合路由（mix）** — 三种情况：

| 情况                | 条件                                       | 情绪来源                   | 强度来源                  | 置信度公式                |
| ------------------- | ------------------------------------------ | -------------------------- | ------------------------- | ------------------------- |
| 混合确认 + 高置信度 | `is_mixed=True` 且 `mix_confidence ≥ 0.65` | Mix 的 primary + secondary | Mix 的 adjusted_intensity | `emotion×0.3 + mix×0.7`   |
| 混合确认 + 低置信度 | `is_mixed=True` 且 `mix_confidence < 0.65` | 回退到 Emotion Agent       | Emotion 的 intensity      | `emotion×0.8`（降低 20%） |
| 未检测到混合        | `is_mixed=False`                           | Emotion Agent              | Emotion 的 intensity      | `emotion×0.9`（降低 10%） |

##### ⑤.3 置信度降低机制

系统在以下情况**主动降低置信度**，反映判断的不确定性：

```
                    置信度
                       │
    直接路由 ──────────│──── emotion_confidence（不变）
                       │
    讽刺确认+高置信 ───│──── emotion×0.3 + sarcasm×0.7（加权合并）
                       │
    讽刺确认+低置信 ───│──── emotion×0.8（降 20%，回退到表层情绪）
                       │
    讽刺未检测 ────────│──── emotion×0.9（降 10%，排除但不确定）
                       │
    混合确认+高置信 ───│──── emotion×0.3 + mix×0.7（加权合并）
                       │
    混合确认+低置信 ───│──── emotion×0.8（降 20%，回退到表层情绪）
                       │
    混合未检测 ────────│──── emotion×0.9（降 10%，排除但不确定）
                       │
```
...
- **20% 惩罚**：虽然检测到了，但置信度不够高，系统选择"保守回退"到表层情绪，同时降低置信度表示不确定性
- **10% 惩罚**：排除了讽刺/混合可能性，但"排除"本身也有不确定性，小幅降低置信度
...
在 `EmotionAnalysisService.java` 中，Judge Agent 的调用位于情绪分析流水线的最后一步：

```java
// 第 30-60 行：analyze() 方法
router->emotion->maybe(sarcasm/mix)->judge

// 第 51 行：Judge Agent 调用
Map<String, Object> judgeResult = agentClient.judge(judgeRequest);
```

Judge Agent 接收所有上游 Agent 的结果，输出最终的 `JudgeResult`，供 Chat Agent 生成回复使用。
...
**历史消息查询逻辑**（`ChatPersistenceService.historyBeforeTurn()`）：

```java
// 查询最近 20 条历史消息（排除当前用户消息）
chatMessageRepository.findTop20ByConversationIdAndIdNotOrderByCreatedAtDesc(
    conversationId, userMessageId);
// 反转顺序，使最早消息在前
Collections.reverse(messages);
// 只返回 role 为 user 和 assistant 的消息
```
...
系统使用 SQLite 数据库，通过 JPA/Hibernate 自动建表（`ddl-auto: update`），共 5 张表：
...
**接口**：`GET /api/auth/captcha` 返回 `{ captcha_key, captcha_image }`

验证码仅在注册时要求，登录不需要。

#### JWT 认证

使用 `jjwt` 库（v0.12.6）实现无状态认证：
...
JWT 验证通过后，将 `userId`、`username`、`role` 存入 request attribute，Controller 直接获取使用。
...
- **JWT 层**：`JwtAuthFilter` 从 Token 中提取角色信息
- **Controller 层**：根据 `request.getAttribute("role")` 判断权限
- **前端层**：`isAdminMode()` 检查角色，管理员看到完全不同的界面布局

```java
// EmotionAnalysisController.java 示例
String effectiveUserId = userId;
if (!"admin".equals(role)) {
    // 普通用户只能看自己的数据
    effectiveUserId = request.getAttribute("userId").toString();
...
> Benchmark 对比图详见 `docs/benchmark_comparison.svg`
